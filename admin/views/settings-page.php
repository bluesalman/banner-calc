    ...
    <form method="post" action="options.php">
        ...
        <div class="bannercalc-section-block">
            ...
            <p class="bannercalc-field-hint" style="margin-bottom:12px;">
                <?php esc_html_e( 'Configure delivery speed tiers. Markup % is a production speed surcharge added as a separate fee in the cart. Shipping cost is the same for all tiers.', 'bannercalc' ); ?>
            </p>
        ...
        <div class="bannercalc-section-block">
            <h2 class="bannercalc-section-heading"><?php esc_html_e( 'Shipping', 'bannercalc' ); ?></h2>
            <p class="bannercalc-field-hint" style="margin-bottom:12px;">
                <?php esc_html_e( 'One shipping cost applies to all delivery speed tiers. Urgency markup is a separate production surcharge — it does not change the shipping cost.', 'bannercalc' ); ?>
            </p>

            <?php
            // Get available WC shipping methods for the single dropdown.
            $wc_shipping_methods = [];
            ...
            $current_method = $settings['shipping_method'] ?? '';
            $current_cost   = $settings['shipping_cost'] ?? 0;
            ?>
            <table class="form-table bannercalc-form-table">
                <tr>
                    <th scope="row">
                        <label for="bannercalc_shipping_method"><?php esc_html_e( 'WC Shipping Method', 'bannercalc' ); ?></label>
                    </th>
                    <td>
                        <select id="bannercalc_shipping_method" name="bannercalc_settings[shipping_method]" style="min-width:300px;">
                            ...
                            <?php foreach ( $wc_shipping_methods as $method_id => $method_label ) : ?>
                                <option value="<?php echo esc_attr( $method_id ); ?>"
                                        <?php selected( $current_method, $method_id ); ?>>
                                    <?php echo esc_html( $method_label ); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <p class="description"><?php esc_html_e( 'The WooCommerce shipping method to use at checkout for delivery orders.', 'bannercalc' ); ?></p>
                    </td>
                </tr>
                <tr>
                    <th scope="row">
                        <label for="bannercalc_shipping_cost"><?php esc_html_e( 'Shipping Cost (£)', 'bannercalc' ); ?></label>
                    </th>
                    <td>
                        <input type="number"
                               id="bannercalc_shipping_cost"
                               name="bannercalc_settings[shipping_cost]"
                               value="<?php echo esc_attr( $current_cost ); ?>"
                               step="0.01" min="0" class="small-text" style="width:100px;" />
                        <p class="description"><?php esc_html_e( 'Displayed on the product page as the estimate. Enter the cost manually since WC formulas cannot be auto-read.', 'bannercalc' ); ?></p>
                    </td>
                </tr>
            </table>
        </div>
